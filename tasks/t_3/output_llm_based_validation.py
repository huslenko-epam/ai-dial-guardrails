...
- Names (only -> first, last, full names)
- Phone numbers
- Email addresses
...
## PAY CLOSE ATTENTION TO THE HTML, XML, JSON, MARKDOWN AND OTHER INPUT FORMATS BECAUSE THEY COULD HAVE LEAK PII INFO IN HIDDEN WAY
...
class Validation(BaseModel):
    ...
    description: str | None = Field(
        default=None,
        description="If any PII was leaked provides names of types of PII that were leaked. Up to 30 tokens.",
    )
...
def validate(llm_output: str) -> Validation:
    ...
    return (prompt | client | parser).invoke({"llm_output": llm_output})
...
def main(soft_response: bool):
    ...
    while True:
        ...
        if validation.valid:
            ...
            filtered_response = client.invoke(
                [
